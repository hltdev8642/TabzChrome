...
1. **Fix security issues in backend** *(Deprioritized - auth token required, low risk)*
...
2. ✅ **Update tool counts in documentation** *(Completed 2025-12-23)*
   - README.md: Change "20" to "29"
   - docs/PLUGIN.md: Change "26" to "29"
   - All skill files: Update to "29"

3. ✅ **Sync `.claude/skills/tabz-guide/`** from `plugins/` *(Completed 2025-12-23)*

### This Week - Medium Priority

4. ✅ **Split `background.ts`** (4,537 lines → 21 files, 13 modules) *(Completed 2025-12-23)*

5. ✅ **Remove CDP references** *(Completed 2025-12-23)*
   - docs/PLUGIN.md - removed CDP setup instructions
   - Error messages in MCP tools - updated to reference TabzChrome

6. **Use existing `createPendingRequest()`** in browser.js routes

7. ✅ **Create shared utilities** *(Partially completed 2025-12-23)*:
   - `modules/path-utils.js` for `expandTilde()` - *not done*
   - ✅ `tabz-mcp-server/src/utils.ts` for `formatBytes()`
...
10. ✅ **Update README version badge** to 1.2.8 *(Completed 2025-12-23)*

11. ✅ **Remove dead code** *(Completed 2025-12-23)*:
    - ✅ `cleanupScreenshots()` in client.ts
    - ✅ `convertPathForWSL()` in client.ts
    - Commented tool groups in index.ts - *not done*

### Also Completed (not in original list)

12. ✅ **Split `client.ts`** in MCP server (1,041 lines → 8 modules) *(Completed 2025-12-23)*
13. ✅ **Updated CLAUDE.md** architecture diagram with new module structure *(Completed 2025-12-23)*
